...
/*
 * Stress ONLY formula-relevant state.
 * No full struct touching.
 * No worksheet metadata noise.
 */
...
/*
 * Key design:
 *  - Parse worksheet once
 *  - Collect ONLY formula cells
 *  - Re-enter parsing multiple times
 *  - Never walk non-formula cells
 */
                             ...
                             size_t raw_sz)
{
    ...
    for (DWORD r = 0; r <= max_r && seen < MAX_FORMULA_CELLS; r++) {
        for (DWORD c = 0; c <= max_c && seen < MAX_FORMULA_CELLS; c++) {
            ...
            stress_formula_cell(cell, raw, raw_sz,
                                (r << 16) ^ c);
            seen++;
        }
    }
